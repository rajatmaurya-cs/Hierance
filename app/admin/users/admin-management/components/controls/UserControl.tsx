...
import { Users, ArrowRight, ShieldCheck, Clock, Settings } from "lucide-react";
...
export default function UserControl({ role }: Props) {
  return (
    <div className="flex flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-gray-900 leading-tight">
              Job Seeker Access Control
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">
              Current Access Level: <span className="font-bold text-sky-600">{role.replace("_", " ")}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-8 bg-gray-50 flex flex-col items-center text-center justify-center">
        ...
        <p className="text-sm text-gray-500 max-w-md mb-8">
          The comprehensive job seeker monitoring and profile control system is hosted in its dedicated module.
        </p>

        <div className="flex gap-4">
          <Link 
            href="/admin/users/job-seekers" 
            className="flex items-center gap-2 px-6 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-sm font-semibold shadow-sm transition-all"
          >
            Go to Job Seekers Module <ArrowRight size={16} />
          </Link>
