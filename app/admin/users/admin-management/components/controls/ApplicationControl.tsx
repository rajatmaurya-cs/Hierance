...
import { FileText, ArrowRight, ShieldCheck, Clock, Settings } from "lucide-react";
...
export default function ApplicationControl({ role }: Props) {
  return (
    <div className="flex flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Current Access Level: <span className="font-bold text-indigo-600">{role.replace("_", " ")}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-8 bg-gray-50 flex flex-col items-center text-center justify-center">
        ...
        <p className="text-sm text-gray-500 max-w-md mb-8">
          The full job application tracking and review system is hosted in its dedicated module to provide maximum screen real estate.
        </p>

        <div className="flex gap-4">
          <Link 
            href="/admin/platform-management/applications/profiles" 
            className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold shadow-sm transition-all"
          >
            Go to Applications Module <ArrowRight size={16} />
          </Link>
