    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-[15px] font-semibold text-slate-800 font-inter">Employment History</h2>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-slate-50/50 border border-slate-100/60 rounded-xl p-5 space-y-4 shadow-sm">
          ...
          <div className="flex justify-between items-center">
            <label className="flex items-center gap-2 text-[13px] text-slate-600 font-inter cursor-pointer">
          ...
          {employments.map((emp) => (
            <div key={emp.id} className="p-4 rounded-xl border border-slate-100 bg-white shadow-[0_1px_4px_-1px_rgba(0,0,0,0.05)] hover:shadow-md transition-all group">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-[15px] font-semibold text-slate-800 font-inter">{emp.title}</h3>
                  <p className="text-[14px] text-slate-600 mt-0.5 font-inter font-medium">{emp.company}</p>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3 text-[13px] text-slate-500 font-inter">
                {emp.location && (
              ...
              {emp.description && (
                <p className="text-[14px] text-slate-600 mt-3 leading-relaxed font-inter">{emp.description}</p>
              )}
            </div>
          ))}
        </div>
      ) : (
        <div className="text-center py-10 text-sm text-slate-400 font-inter">No employment history added yet.</div>
      )}
    </div>
  );
};
