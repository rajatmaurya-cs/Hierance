    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-[15px] font-semibold text-slate-800 font-inter">Education</h2>
        </div>
        <button
      ...
      {educations.length > 0 ? (
          ...
          {educations.map((edu) => (
            <div key={edu.id} className="p-4 rounded-xl border border-slate-100 bg-white shadow-[0_1px_4px_-1px_rgba(0,0,0,0.05)] hover:shadow-md transition-all group">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-[15px] font-semibold text-slate-800 font-inter">{edu.degree}</h3>
                  <p className="text-[14px] text-slate-600 mt-0.5 font-inter font-medium">{edu.institution}</p>
                </div>
        ...
        <div className="text-center py-10 text-sm text-slate-400 font-inter">No education added yet.</div>
      )}
    </div>
  );
};
