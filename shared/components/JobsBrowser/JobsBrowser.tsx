...
interface JobsBrowserProps {
  filterType: "category" | "company";
  filterValue: string;
}

export default function JobsBrowser({ filterType, filterValue }: JobsBrowserProps) {
  ...
  const baseJobs = mockJobs.filter(job => {
  ...
  // Display Jobs State
  const [filteredJobs, setFilteredJobs] = useState<any[]>(baseJobs);
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [searchQuery, selectedTypes, selectedCategories, salaryRange, location, postedDay, filterValue, filterType]);
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-32 pb-24">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-400 mb-8">
          ...
          <button onClick={() => router.push("/jobs")} className="hover:text-indigo-600 transition-colors">All Jobs</button>
          <ChevronRight size={14} />
          <span className="text-slate-600 font-medium capitalize">{filterValue}</span>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
              <div>
                <h1 className="text-4xl font-normal text-slate-800 tracking-tight capitalize">
                  {filterValue} <span className="text-indigo-600/70">Jobs</span>
                </h1>
